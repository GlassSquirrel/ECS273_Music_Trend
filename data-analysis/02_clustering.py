...
Produces:
  figures/cluster_elbow.png       — inertia + silhouette vs k
  figures/cluster_pca.png         — PCA 2D scatter colored by cluster
  figures/cluster_umap.png        — UMAP 2D scatter colored by cluster (if umap installed)
  figures/cluster_profile.png     — normalised feature heatmap per cluster
  figures/cluster_radar.png       — radar chart of cluster audio signatures
  results/cluster_labels.csv      — original data + cluster assignment
"""
...
from config import DATA_PATH, FIGURES_DIR, RESULTS_DIR, CLUSTER_FEATURES, PALETTE, STYLE, FIG_DPI
...
def load_features() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # circular encoding for musical key (0–11 → sin/cos)
...
def preprocess(feat_df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    # PCA: keep 95% variance (for use inside KMeans)
...
def find_optimal_k(X_pca: np.ndarray, k_range=range(2, 13)) -> int:
    ...
    fig.suptitle("Choosing the Number of Clusters (k)", fontsize=13, fontweight="bold")

    ks = list(k_range)
    ax1.plot(ks, inertias, "o-", color="#2980b9", lw=2, ms=7)
    ax1.set_xlabel("k", fontsize=11); ax1.set_ylabel("Inertia (within-cluster SSE)", fontsize=11)
    ax1.set_title("Elbow Method", fontsize=12)
    ax1.xaxis.set_major_locator(plt.MultipleLocator(1))

    ax2.plot(ks, silhouettes, "s-", color="#e74c3c", lw=2, ms=7)
    ax2.set_xlabel("k", fontsize=11); ax2.set_ylabel("Silhouette Score", fontsize=11)
    ax2.set_title("Silhouette Score", fontsize=12)
    ax2.xaxis.set_major_locator(plt.MultipleLocator(1))

    best_k = ks[int(np.argmax(silhouettes))]
    ax2.axvline(best_k, color="#e74c3c", linestyle="--", lw=1.5,
                label=f"Best k = {best_k}")
    ax2.legend(fontsize=10)

    plt.tight_layout()
    path = f"{FIGURES_DIR}/cluster_elbow.png"
...
def plot_pca(X_scaled: np.ndarray, labels: np.ndarray, k: int):
    ...
    for c_idx in range(k):
        ...
        ax.scatter(coords[mask, 0], coords[mask, 1],
                   color=colors[c_idx], alpha=0.5, s=8, rasterized=True,
    ...
    ax.set_title(f"KMeans Clusters in PCA Space  (k = {k})", fontsize=13, fontweight="bold")
    ax.legend(markerscale=3, fontsize=10, framealpha=0.8,
              loc="best", ncol=2 if k > 6 else 1)

    path = f"{FIGURES_DIR}/cluster_pca.png"
    fig.savefig(path, dpi=FIG_DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  saved → {path}")
...
def plot_umap(X_scaled: np.ndarray, labels: np.ndarray, k: int):
    ...
    ax.set_title(f"KMeans Clusters in UMAP Space  (k = {k})", fontsize=13, fontweight="bold")
    ax.legend(markerscale=3, fontsize=10, framealpha=0.8,
              loc="best", ncol=2 if k > 6 else 1)
    ax.set_xticks([]); ax.set_yticks([])

    path = f"{FIGURES_DIR}/cluster_umap.png"
    fig.savefig(path, dpi=FIG_DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  saved → {path}")
...
def plot_profile(df: pd.DataFrame, labels: np.ndarray, k: int):
    """Z-score feature means per cluster — shows what defines each cluster."""
    # interpretable features for profile display
    profile_cols = [
        "tempo", "loudness", "duration", "sections_count",
        "avg_timbre_0", "avg_timbre_1", "avg_timbre_2", "avg_timbre_3",
        "avg_pitch_0", "avg_pitch_3", "avg_pitch_7",  # tonic, mediant, dominant
    ...
    # z-score across clusters (column-wise)
    ...
    ax.set_title("Cluster Audio Profiles (normalised feature means)", fontsize=13, fontweight="bold")
    ax.set_ylabel("Cluster", fontsize=11)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=35, ha="right", fontsize=9)

    plt.tight_layout()
    path = f"{FIGURES_DIR}/cluster_profile.png"
    fig.savefig(path, dpi=FIG_DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  saved → {path}")
...
def plot_radar(df: pd.DataFrame, labels: np.ndarray, k: int):
    radar_cols = ["tempo", "loudness", "duration", "sections_count",
                  "avg_timbre_0", "avg_timbre_1"]
    ...
    # min-max scale to [0, 1] per column
    ...
    for c_idx, row in normed.iterrows():
        ...
        ax.plot(angles, vals, lw=2, label=f"Cluster {c_idx}",
                color=cmap(c_idx / k))
    ...
    short_labels = ["Tempo", "Loudness", "Duration", "Sections",
                    "Timbre-0\n(Power)", "Timbre-1\n(Bright)"][:N]
    ax.set_xticklabels(short_labels, fontsize=10)
    ax.set_yticks([0.25, 0.5, 0.75])
    ax.set_yticklabels(["25%", "50%", "75%"], fontsize=8)
    ax.set_title("Cluster Audio Signature (Radar)", fontsize=13,
                 fontweight="bold", pad=20)
    ax.legend(loc="upper right", bbox_to_anchor=(1.35, 1.1), fontsize=10)

    plt.tight_layout()
    path = f"{FIGURES_DIR}/cluster_radar.png"
    fig.savefig(path, dpi=FIG_DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  saved → {path}")
...
def save_labels(df: pd.DataFrame, labels: np.ndarray):
    out = df[["track_id", "title", "artist_name", "year",
              "tempo", "loudness", "duration", "song_hotttnesss"]].copy()
    out["cluster"] = labels
    path = f"{RESULTS_DIR}/cluster_labels.csv"
    ...
    for col in ["tempo", "loudness", "duration", "song_hotttnesss"]:
    ...
    grp = summary.groupby("cluster").agg(
        ...
        avg_hotness=("song_hotttnesss", "mean"),
    ).round(3)
    print("\nCluster summary:")
    print(grp.to_string())
...
if __name__ == "__main__":
    ...
    # allow manual override here if desired
    K = best_k
    print(f"  → Using k = {K}")
    ...
    print("\nClustering complete. Figures → ", FIGURES_DIR)
