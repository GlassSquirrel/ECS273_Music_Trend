...
def compute_umap(latent_vectors, n_components=3):
    """单独的UMAP降维，支持2D或3D，只跑一次"""
    print(f"Running UMAP {n_components}D (this may take ~1-2 min on CPU)...")
...
def visualize_latent(latent_vectors, labels, df_clean, save_prefix="viz", coords_2d=None):
    """UMAP 3D 交互散点图（plotly）+ 时间趋势堆叠图"""
    import plotly.express as px

    # ── 7.1 UMAP 投影 ────────────────────────────
    if coords_2d is None:
        coords = compute_umap(latent_vectors, n_components=3)
    else:
        coords = coords_2d
    ...
    print(f"Saved: {html_path}  (open in browser to interact)")

    # ── 7.3 同时保存静态2D截图（需要kaleido，可选）────
    try:
        fig2d_path = f"{save_prefix}_umap_3d_static.png"
        fig.write_image(fig2d_path, width=900, height=700)
        print(f"Saved: {fig2d_path}")
    except Exception:
        print("(静态PNG需要 pip install kaleido，跳过)")
...
def main():
    ...
    print("\nTop artist terms per cluster:")
    from collections import Counter
    for c in sorted(df_clean["cluster"].unique()):
        subset = df_clean[df_clean["cluster"] == c]
        all_terms = []
        for terms in subset["artist_terms"].dropna():
            all_terms.extend([t.strip() for t in str(terms).split(";") if t.strip()])
        top5 = [t for t, _ in Counter(all_terms).most_common(5)]
    ...
    print("  latent_vectors.npy       — VAE latent 向量 (N, 32)")
    print("  vae_weights.pt           — 模型权重")
    print("  training_history.png     — 训练曲线 (acoustic/lyric/terms/kl)")
    print("  cluster_selection.png    — Elbow + Silhouette (高维32D cosine)")
    print("  viz_umap_3d.html         — UMAP 3D 交互散点图（浏览器打开）")
    print("  viz_umap_3d_static.png   — UMAP 3D 静态截图（需要kaleido）")
    print("  viz_decade_trend.png     — Cluster 时间趋势图 (ThemeRiver数据基础)")
