...
def load_and_preprocess(path: str):
    ...
    # def parse_lyric_text(row):
    #     if row["lyrics_available"] != 1:
    #         return ""
    #     if not isinstance(row["lyrics_top5_words"], str):
    #         return ""
        
    #     words  = row["lyrics_top5_words"].split(";")
        
    #     # 如果有counts，按频次重复词语，让TF-IDF感知到词频
    #     if isinstance(row["lyrics_top5_counts"], str):
    #         try:
    #             counts = [int(float(c)) for c in row["lyrics_top5_counts"].split(";")]
    #             # 每个词重复min(count, 10)次，避免某个词占比过大
    #             tokens = []
    #             for w, c in zip(words, counts):
    #                 tokens.extend([w] * min(c, 10))
    #             return " ".join(tokens)
    #         except:
    #             pass
    ...
    # lyric_scaler = MaxAbsScaler()  # 用MaxAbs而不是Standard，因为稀疏矩阵有大量0
    # lyric_tfidf = lyric_scaler.fit_transform(lyric_tfidf).astype(np.float32)

    # ── 1.4 歌词 BoW 特征（完整1000词 TF-IDF，来自 musiXmatch 完整数据）────
    ...
    print(f"Lyric BoW features: {lyric_tfidf.shape}")
    print(f"Lyric non-zero ratio: {(lyric_tfidf != 0).mean():.4f}")
...
def vae_loss(x_a, x_l, x_a_hat, x_l_hat, mu, logvar,
    ...
    if lyric_mask.sum() > 0:
        ...
        # 加这两行保护
        if torch.isnan(recon_l) or torch.isinf(recon_l):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x_a, x_l, has_lyr in loader:
            ...
            loss, ra, rl, kl = vae_loss(x_a, x_l, x_a_hat, x_l_hat,
                                         mu, logvar, has_lyr, beta=beta, lyric_weight=0.1)
            loss.backward()
...
def find_optimal_k(coords_2d, k_range=range(3, 11)):
    """在UMAP 2D坐标上用 Elbow + Silhouette 选最优 K"""
    inertias, silhouettes = [], []
    for k in k_range:
        km = KMeans(n_clusters=k, random_state=RANDOM_SEED, n_init=10)
        labels = km.fit_predict(coords_2d)
        inertias.append(km.inertia_)
        silhouettes.append(silhouette_score(coords_2d, labels,
                                            sample_size=min(2000, len(coords_2d))))
...
def compute_umap(latent_vectors):
    ...
    reducer = umap.UMAP(
        n_components=2,
        n_neighbors=15,   # 从30降到15，更关注局部结构
...
def run_clustering(coords_2d, n_clusters=N_CLUSTERS):
    """直接在已有的UMAP 2D坐标上聚类"""
...
def main():
    ...
    print("Finding optimal k on UMAP 2D...")
    best_k = find_optimal_k(coords_2d, k_range=range(3, 11))

    print(f"\nRunning KMeans with k={best_k}...")
    labels, km = run_clustering(coords_2d, n_clusters=best_k)

    # ── Step 7: 可视化（coords_2d直接复用）────────────
    print("\n" + "="*50)
